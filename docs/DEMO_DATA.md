...
- **스키마 기준**: `supabase/migrations/0001` ~ `0018` 전부 적용된 상태 (성별·등급·완료여부
  필드, 거래구분·거래품목 정수 코드, `customer_events` 테이블 포함).
...
-- (4) 등급 분포
select coalesce(grade, '미지정') as grade, count(*)
from customers where owner_id = '<UUID>' and memo like '[DEMO]%'
group by grade order by grade;
-- 기대: VIP 3 / 우수 3 / 일반 3 / 신규 2 / 미지정 7
...
| # | 이름 | 성별 | 등급 | 유입경로 | 방문목적 | 최초거래일 | 마지막연락 | 거래 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| c01 | 데모 강토리 | 여성 | VIP | 지인추천 | 돌제품 | 30일 전 | 30일 전 | 1 |
| c02 | 데모 남해린 | 여성 | 미지정 | 네이버플레이스 | 돌제품 | 190일 전 | 190일 전 | **0** |
| c03 | 데모 도하준 | 남성 | 우수 | 당근마켓 | 돌제품·골드바 | **없음** | **없음** | 2 (같은 날) |
| c04 | 데모 류가온 | 모름 | 미지정 | 워크인 | 돌제품·기타 | 95일 전 | 95일 전 | 1 |
| c05 | 데모 문서아 | 여성 | 일반 | 기타 | 돌제품·주얼리맞춤 | 20일 전 | 20일 전 | 1 |
| c06 | 데모 박도윤 | 남성 | VIP | 네이버플레이스 | 골드바 | 100일 전 | 100일 전 | 2 |
| c07 | 데모 서지호 | 남성 | 신규 | 지인추천 | 골드바 | 10일 전 | 10일 전 | 1 |
| c08 | 데모 안유찬 | 남성 | 미지정 | 당근마켓 | 골드바·매입 | **없음** | **없음** | 2 |
| c09 | 데모 오세라 | 여성 | 일반 | 워크인 | 골드바 | 5일 전 | 5일 전 | 1 |
| c10 | 데모 유하람 | 모름 | 미지정 | 기타 | 골드바 | **없음** | **없음** | **0** |
| c11 | 데모 이준서 | 남성 | 신규 | 당근마켓 | 매입 | 40일 전 | 40일 전 | 1 |
| c12 | 데모 임채원 | 여성 | 우수 | 네이버플레이스 | 매입 | 60일 전 | 60일 전 | 2 |
| c13 | 데모 장시우 | 남성 | 미지정 | 지인추천 | 매입 | **없음** | **없음** | 2 |
| c14 | 데모 전보름 | 여성 | 미지정 | 워크인 | 매입·기타 | 35일 전 | 35일 전 | 1 |
| c15 | 데모 정해원 | 남성 | VIP | 기타 | 매입 | 380일 전 | 380일 전 | 2 |
| c16 | 데모 조가율 | 여성 | 우수 | 네이버플레이스 | 주얼리맞춤 | 15일 전 | 15일 전 | 2 (같은 날) |
| c17 | 데모 채민재 | 남성 | 미지정 | 지인추천 | 주얼리맞춤 | 55일 전 | 55일 전 | 1 |
| c18 | 데모 한소율 | 여성 | 일반 | 당근마켓 | 주얼리맞춤·골드바 | **없음** | **없음** | 2 |

구성 요약:
- 방문목적: 돌제품 5 / 골드바 7 / 매입 6 / 주얼리맞춤 4 / 기타 2 (복수 목적 6명: c03,c04,c05,c08,c14,c18)
- 유입경로: 당근마켓 4 / 네이버플레이스 4 / 지인추천 4 / 워크인 3 / 기타 3
- 등급: VIP 3 / 우수 3 / 일반 3 / 신규 2 / 미지정 7
- 성별: 남성 8 / 여성 8 / 모름 2
- **거래 없음 2명** (c02, c10)
...
| e02 | c03 도하준 | 재방문 | 오늘 | 7일 이내 |
| e03 | c05 문서아 | 맞춤주문 | +25일 | 30일 이내 |
| e04 | c08 안유찬 | 재방문 | -10일 | 기한 지남 |
| e05 | c16 조가율 | 생일 | +2일 | 7일 이내 |
| e06 | c17 채민재 | 안부 | -20일 | 기한 지남 |

기한 지남 2 / 7일 이내 3 / 30일 이내 1 → 기본 목록(지남+30일 이내) 합계 **6건 전체**.
...
| 방문목적별 | 돌제품 5 · 골드바 7 · 매입 6 · 주얼리맞춤 4 · 기타 2 | 합계 24 ≠ 18 (복수 목적) |
| 홈 일정 위젯(기본, 지남+30일 이내) | **6건** | §6 참고 |
| 최근 거래 5건 | 이번 달 거래로 채워짐, 거래일 내림차순 | |
