...
export function EventsSection({
  ...
  trades: { id: string; trade_date: string; item_detail: string | null }[];
}) {
  ...
  const tradeLabelById = new Map(
    trades.map((t) => [t.id, `${formatKoreanDate(t.trade_date)} 거래`]),
  );
          ...
          <form action={formAction} className="mt-4 flex flex-col gap-4 border-t pt-4">
            ...
            {trades.length > 0 ? (
              <div className="flex flex-col gap-1.5">
                ...
                <Select name="trade_id" items={Object.fromEntries(trades.map((t) => [t.id, tradeLabelById.get(t.id) ?? t.id]))} defaultValue={v?.trade_id || undefined}>
                  <SelectTrigger className="w-full">
                  ...
                  <SelectContent>
                    {trades.map((t) => (
                      <SelectItem key={t.id} value={t.id}>
                        {tradeLabelById.get(t.id)}
                        {t.item_detail ? ` (${t.item_detail})` : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
